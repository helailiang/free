...
class R2GuiCircleProducer:
    ...
    消费侧通过 ``wait_newer_than(seq, timeout)`` 阻塞直到 ``seq`` 增大（即又组完一整圈），
    再调用 ``R2SingleScanRadar.measurement_from_full_triples`` 做索引起止与距离窗裁剪，
    避免与 ``optimized_single_measurement`` 在同一步内交替 ``read`` 同一 ``socket``。
    ...
    def stop_and_join(self, join_s: float = 6.0) -> None:
        """请求退出并等待后台线程结束（可能阻塞在最后一次 ``assemble_one_full_scan`` 直至 deadline）。"""
        self._stop.set()
    ...
    def current_seq(self) -> int:
        """已成功组圈并发布的单调序号；首次 ``wait_newer_than(0,…)`` 在第一次组圈完成后返回。"""
        with self._lock:
            return int(self._seq)

    def wait_newer_than(self, seq: int, timeout_s: float) -> tuple[list[tuple[int, int, int]], int]:
        """
        阻塞直到内部 ``_seq > seq`` 或超时/致命错误。

        Returns:
            ``(triples 快照副本, 新的 _seq)``，便于调用方把 ``seq`` 游标推进。
        """
    ...
    def _run_loop(self) -> None:
        n_pts = int(getattr(self._radar, "_points_per_circle", R2_FULL_CIRCLE_POINT_COUNT))
...
class R2SequenceThread(QThread):
    ...
    def _continuous_fetch_measurement(
        ...
        """
        从 ``R2GuiCircleProducer`` 取 **严整新一圈** 后，经 ``measurement_from_full_triples`` 生成与
        ``optimized_single_measurement`` 同结构的字典；并推进 ``seq_gate[0]`` 为返回的 seq。
        """
    ...
    def _run_sequence(
        ...
        try:
            ...
            for k in range(1, cfg.steps + 1):
                ...
                # 降低步进刚结束立刻取一圈时的机械扰动对测距/反射主峰的影响（与 settle_ms 叠加，可再调大 UI 等待）。
                ...
                # 振动段内后台线程已不断组圈；此处把「游标」拨到当前最新 seq，后续 wait 只认**转台后**新闭合的整圈。
...
class R2ProbeReflectThread(QThread):
    ...
    def run(self) -> None:
        ...
        full_hi = int(R2_FULL_CIRCLE_POINT_COUNT) - 1
...
class R2ResolutionMainWindow(QMainWindow):
    ...
    def _on_probe_max_reflect(self) -> None:
        ...
        self._log(
            ...
            f"整圈 index 0…{int(R2_FULL_CIRCLE_POINT_COUNT) - 1}（与索引起止/连续高反±index 无关）；"
