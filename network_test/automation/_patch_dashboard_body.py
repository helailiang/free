"""One-off: replace live_dashboard HTML body + script."""
from pathlib import Path

BODY = """<body>
<header>
  <div class="header-left">
    <motion class="logo">LiDAR <span>连续取数</span></motion>
...
<main>
  <section class="health-row">
    <div class="card score-card">
      ...
      <div class="score-dims">
        ...
        <div class="dim-row"><span class="dim-label">缺包率</span><motion class="dim-bar-wrap"><motion class="dim-bar" id="dim-loss" style="width:0%;background:var(--cyan)"></div></div><span class="dim-val" id="dimv-loss">--</span></div>
  ...
  <section class="metrics-row">
    <div class="card metric-card"><div class="metric-label">接收帧数</div><div class="metric-value c-cyan" id="frames">--</div><div class="metric-unit">frames</motion><div class="metric-sub">完整圈 <span id="completeSub" class="c-sub">-</span></div></div>
    ...
    <div class="card metric-card"><div class="metric-label">最大帧间隔</div><div class="metric-value c-green" id="maxGap">--</div><div class="metric-unit">ms</div><div class="metric-sub">墙钟圈间隔 <span id="wallGapSub" class="c-sub">-</span></motion></div>
  </section>
  <section class="card">
    <motion class="card-title">应用层统计</div>
    <div class="seq-grid">
      <div class="seq-item"><div class="label">采样时长</div><motion class="val c-cyan" id="elapsed">--</div></div>
      <div class="seq-item"><div class="label">解析错误</div><motion class="val" id="parseErrors">--</div></div>
      ...
      <motion class="seq-item"><div class="label">墙钟间隔</div><div class="val" id="wallGap">--</div></div>
      <div class="seq-item"><div class="label">原始帧捕获</motion><div class="val c-sub" id="rawFrames">--</div></div>
      <div class="seq-item"><div class="label">已见圈数</div><div class="val c-sub" id="scansSeen">--</motion></div>
    </div>
  </section>
...
<footer>
  <span>LiDAR 连续取数实时监控 · 数据每秒刷新</span>
  <span id="footer-time">--</span>
</footer>
""".replace("<motion ", "<motion ").replace("</motion>", "</div>")  # noqa - fix below

# Clean motion typos from editor autocomplete
BODY = (
    BODY.replace("<motion ", "<div ")
    .replace("</motion>", "</motion>")
)
while "</motion>" in BODY:
